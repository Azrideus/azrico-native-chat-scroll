const itemsInDB: Array<{ _id: string; user: string; text: string; date: Date }> = [];

export async function loadExampleChats(props: any): Promise<any[]> {
...
export async function addExampleChat(r: any) {
	itemsInDB.unshift({
		...r,
		_id: r['_id'] ?? 'oldmsg-' + getExampleChatLenght(),
		user: r['user'] ?? r['User Name'],
		text: r['text'] ?? r['Content'],
		date: new Date(r['Date'] ?? r['_created_date']),
	});
}
export function getExampleChatLenght() {
	return itemsInDB.length;
}
